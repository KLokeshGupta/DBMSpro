...
int Schema::dropIndex(char *relName, char *attrName) {
    // if the relName is either Relation Catalog or Attribute Catalog,
     ...
     if(!strcmp(relName,RELCAT_RELNAME) or !strcmp(relName,ATTRCAT_RELNAME)) return E_NOTPERMITTED;
...
int Schema::createRel(char relName[],int nAttrs, char attrs[][ATTR_SIZE],int attrtype[]){
    ...
    char relName1[ATTR_SIZE];
    strcpy(relName1,"RelName");
    targetRelId=BlockAccess::linearSearch(RELCAT_RELID,relName1,relNameAsAttribute,EQ);
    // if a relation with name `relName` already exists  ( linearSearch() does
    //                                                     not return {-1,-1} )
    //     return E_RELEXIST;
    if(targetRelId.block!=-1 or targetRelId.slot!=-1) return E_RELEXIST;
